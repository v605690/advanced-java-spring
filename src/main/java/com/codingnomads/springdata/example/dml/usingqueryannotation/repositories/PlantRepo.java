...
public interface PlantRepo extends JpaRepository<Plant, Long> {
    ...
    @Query("SELECT p, st FROM Plant p JOIN SoilType st ON p.favoriteSoilType.id = st.id WHERE st.name = :soilTypeName AND p.fruitBearing = :fruitBearing")
    ArrayList<Plant> getPlantsBasedOnSoilTypeAndFruitBearing(@Param("fruitBearing") boolean fruitBearing, @Param("soilTypeName") String soilTypeName);
    ...
    @Query("SELECT p, st FROM Plant p JOIN SoilType st ON p.favoriteSoilType.id = st.id WHERE p.sunType = ?1 AND p.fruitBearing = ?2 AND st.ph > ?3")
    ArrayList<Plant> getPlantsBasedOnSunTypeAndFruitBearingAndPhGreaterThan(String sunType, boolean fruitBearing, double lowerPhBound, Sort sort);
    ...
    @Query(
            ...
            countQuery = "SELECT count(p.id) FROM Plant p JOIN SoilType st ON p.favoriteSoilType.id = st.id WHERE ph < ?1"
    )
