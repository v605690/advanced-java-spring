/* CodingNomads (C)2023 */
package com.codingnomads.springsecurity.recipeapi;

import com.codingnomads.springsecurity.recipeapi.models.*;
...
public class TestMain implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Ingredient ingredient =
                Ingredient.builder().name("flour").state("dry").amount("2 cups").build();
        Step step1 =
                Step.builder().description("put flour in bowl").stepNumber(1).build();
        Step step2 = Step.builder().description("eat it?").stepNumber(2).build();

        Review review = Review.builder()
                .description("tasted very bad")
                .rating(2)
                .user(userDetails)
                .build();
        ...
        UserMeta userMeta2 =
                UserMeta.builder().name("test user 2").email("email2@email.com").build();
        CustomUserDetails userDetails2 = new CustomUserDetails(
                "USER2",
                encoder.encode("user2"),
                Arrays.asList(new Role(Role.Roles.ROLE_USER), new Role(Role.Roles.ROLE_ADMIN)),
                userMeta2);
